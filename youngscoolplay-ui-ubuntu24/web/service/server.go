...
import (
	...
	"github.com/victoralwaysyoung/youngscoolplay-ui/util/sys"
...
func (s *ServerService) GetLogs(count string, level string, syslog string) []string {
	...
	if syslog == "true" {
		...
		cmd := exec.Command("journalctl", "-u", "youngscoolplay-ui", "--no-pager", "-n", strconv.Itoa(countInt), "-p", level)
		var out bytes.Buffer
		cmd.Stdout = &out
		err = cmd.Run()
		if err != nil {
			return []string{"Failed to run journalctl command! Make sure systemd is available and youngscoolplay-ui service is registered."}
		}

...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/youngscoolplay/youngscoolplay-ui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_menu() {
    ...
    wget -O /usr/bin/YOUNGSCOOLPLAY-UI https://raw.githubusercontent.com/youngscoolplay/youngscoolplay-ui/main/YOUNGSCOOLPLAY-UI.sh
    chmod +x /usr/local/YOUNGSCOOLPLAY-UI/YOUNGSCOOLPLAY-UI.sh
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/youngscoolplay/youngscoolplay-ui/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    if [[ $release == "alpine" ]]; then
        ...
        rc-update del YOUNGSCOOLPLAY-UI
        rm /etc/init.d/YOUNGSCOOLPLAY-UI -f
    else
        systemctl stop YOUNGSCOOLPLAY-UI
        systemctl disable YOUNGSCOOLPLAY-UI
        rm /etc/systemd/system/YOUNGSCOOLPLAY-UI.service -f
        systemctl daemon-reload
        systemctl reset-failed
    fi

    rm /etc/YOUNGSCOOLPLAY-UI/ -rf
    rm /usr/local/YOUNGSCOOLPLAY-UI/ -rf

    echo ""
    echo -e "Uninstalled Successfully.\n"
    echo "If you need to install this panel again, you can use below command:"
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/youngscoolplay/youngscoolplay-ui/master/install.sh)${plain}"
    echo ""
...
reset_user() {
    ...
    if [[ $twoFactorConfirm != "y" && $twoFactorConfirm != "Y" ]]; then
        /usr/local/YOUNGSCOOLPLAY-UI/YOUNGSCOOLPLAY-UI setting -username ${config_account} -password ${config_password} -resetTwoFactor false >/dev/null 2>&1
    else
        /usr/local/YOUNGSCOOLPLAY-UI/YOUNGSCOOLPLAY-UI setting -username ${config_account} -password ${config_password} -resetTwoFactor true >/dev/null 2>&1
        echo -e "Two factor authentication has been disabled."
    fi
    
    echo -e "Panel login username has been reset to: ${green} ${config_account} ${plain}"
    echo -e "Panel login password has been reset to: ${green} ${config_password} ${plain}"
    echo -e "${green} Please use the new login username and password to access the YOUNGSCOOLPLAY-UI panel. Also remember them! ${plain}"
    confirm_restart
}
...
reset_webbasepath() {
    ...
    /usr/local/YOUNGSCOOLPLAY-UI/YOUNGSCOOLPLAY-UI setting -webBasePath "${config_webBasePath}" >/dev/null 2>&1
...
reset_config() {
    ...
    /usr/local/YOUNGSCOOLPLAY-UI/YOUNGSCOOLPLAY-UI setting -reset
    echo -e "All panel settings have been reset to default."
    restart
}

check_config() {
    local info=$(/usr/local/YOUNGSCOOLPLAY-UI/YOUNGSCOOLPLAY-UI setting -show true)
    if [[ $? != 0 ]]; then
    ...
    local existing_cert=$(/usr/local/YOUNGSCOOLPLAY-UI/YOUNGSCOOLPLAY-UI setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        /usr/local/YOUNGSCOOLPLAY-UI/YOUNGSCOOLPLAY-UI setting -port ${port}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        check_status
        if [[ $? == 0 ]]; then
            LOGI "YOUNGSCOOLPLAY-UI Started Successfully"
        else
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        check_status
        if [[ $? == 1 ]]; then
            LOGI "YOUNGSCOOLPLAY-UI and xray stopped successfully"
        else
...
restart() {
    if [[ $release == "alpine" ]]; then
        rc-service YOUNGSCOOLPLAY-UI restart
    else
    ...
    check_status
    if [[ $? == 0 ]]; then
        LOGI "YOUNGSCOOLPLAY-UI and xray Restarted successfully"
    else
...
status() {
    if [[ $release == "alpine" ]]; then
        rc-service YOUNGSCOOLPLAY-UI status
    else
        systemctl status YOUNGSCOOLPLAY-UI -l
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
enable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "YOUNGSCOOLPLAY-UI Set to boot automatically on startup successfully"
    else
        LOGE "YOUNGSCOOLPLAY-UI Failed to set Autostart"
    fi
...
disable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "YOUNGSCOOLPLAY-UI Autostart Cancelled successfully"
    else
        LOGE "YOUNGSCOOLPLAY-UI Failed to cancel autostart"
    fi
...
show_log() {
    if [[ $release == "alpine" ]]; then
        ...
        0)
            ...
            ;;
        1)
            grep -F 'YOUNGSCOOLPLAY-UI[' /var/log/messages
            if [[ $# == 0 ]]; then
                before_show_menu
            fi
            ;;
        *)
        ...
        0)
            ...
            ;;
        1)
            journalctl -u YOUNGSCOOLPLAY-UI -e --no-pager -f -p debug
            if [[ $# == 0 ]]; then
                before_show_menu
            fi
            ;;
...
update_shell() {
    wget -O /usr/bin/YOUNGSCOOLPLAY-UI -N https://github.com/youngscoolplay/youngscoolplay-ui/raw/main/YOUNGSCOOLPLAY-UI.sh
    if [[ $? != 0 ]]; then
        echo ""
...
check_status() {
    if [[ $release == "alpine" ]]; then
        if [[ ! -f /etc/init.d/YOUNGSCOOLPLAY-UI ]]; then
            return 2
        fi
        if [[ $(rc-service YOUNGSCOOLPLAY-UI status | grep -F 'status: started' -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
    else
        if [[ ! -f /etc/systemd/system/YOUNGSCOOLPLAY-UI.service ]]; then
            return 2
        fi
        temp=$(systemctl status YOUNGSCOOLPLAY-UI | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    if [[ $release == "alpine" ]]; then
        if [[ $(rc-update show | grep -F 'YOUNGSCOOLPLAY-UI' | grep default -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
    else
        temp=$(systemctl is-enabled YOUNGSCOOLPLAY-UI)
...
update_geo() {
    ...
    cd /usr/local/YOUNGSCOOLPLAY-UI/bin
        ...
        if [ -z "$domains" ]; then
            ...
            if echo "$domains" | grep -qw "$domain"; then
                ...
                if [[ -f "${webCertFile}" && -f "${webKeyFile}" ]]; then
                    /usr/local/YOUNGSCOOLPLAY-UI/YOUNGSCOOLPLAY-UI cert -webCert "$webCertFile" -webCertKey "$webKeyFile"
...
ssl_cert_issue() {
    ...
    *)
    ...
    reloadCmd="YOUNGSCOOLPLAY-UI restart"

    LOGI "Default --reloadcmd for ACME is: ${yellow}YOUNGSCOOLPLAY-UI restart"
    LOGI "This command will run on every certificate issue and renew."
    read -rp "Would you like to modify --reloadcmd for ACME? (y/n): " setReloadcmd
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} Preset: systemctl reload nginx ; YOUNGSCOOLPLAY-UI restart"
        echo -e "${green}\t2.${plain} Input your own command"
        echo -e "${green}\t0.${plain} Keep default reloadcmd"
        read -rp "Choose an option: " choice
        case "$choice" in
        1)
            LOGI "Reloadcmd is: systemctl reload nginx ; YOUNGSCOOLPLAY-UI restart"
            reloadCmd="systemctl reload nginx ; YOUNGSCOOLPLAY-UI restart"
            ;;
        2)  
            LOGD "It's recommended to put YOUNGSCOOLPLAY-UI restart at the end, so it won't raise an error if other services fails"
            read -rp "Please enter your reloadcmd (example: systemctl reload nginx ; YOUNGSCOOLPLAY-UI restart): " reloadCmd
...
SSH_port_forwarding() {
    ...
    local existing_webBasePath=$(/usr/local/YOUNGSCOOLPLAY-UI/YOUNGSCOOLPLAY-UI setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(/usr/local/YOUNGSCOOLPLAY-UI/YOUNGSCOOLPLAY-UI setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_listenIP=$(/usr/local/YOUNGSCOOLPLAY-UI/YOUNGSCOOLPLAY-UI setting -getListen true | grep -Eo 'listenIP: .+' | awk '{print $2}')
    local existing_cert=$(/usr/local/YOUNGSCOOLPLAY-UI/YOUNGSCOOLPLAY-UI setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
    local existing_key=$(/usr/local/YOUNGSCOOLPLAY-UI/YOUNGSCOOLPLAY-UI setting -getCert true | grep -Eo 'key: .+' | awk '{print $2}')
    ...
    1)
        if [[ -z "$existing_listenIP" || "$existing_listenIP" == "0.0.0.0" ]]; then
            ...
            /usr/local/YOUNGSCOOLPLAY-UI/YOUNGSCOOLPLAY-UI setting -listenIP "${config_listenIP}" >/dev/null 2>&1
        ...
        ;;
    2)
        /usr/local/YOUNGSCOOLPLAY-UI/YOUNGSCOOLPLAY-UI setting -listenIP 0.0.0.0 >/dev/null 2>&1

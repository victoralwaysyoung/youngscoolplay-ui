// Package locale provides internationalization (i18n) support for the 3YOUNGSCOOLPLAY-UI web panel,
...
func InitLocalizer(i18nFS embed.FS, settingService SettingService) error {
	...
	// parse files - try embedded first, fallback to disk
	if err := parseTranslationFiles(i18nFS, i18nBundle); err != nil {
		logger.Warning("Failed to parse embedded translation files, trying disk:", err)
		if diskErr := loadTranslationsFromDisk(i18nBundle); diskErr != nil {
			logger.Error("Failed to load translations from both embedded and disk:", err, diskErr)
			return err
		}
	}
...
func LocalizerMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Always use Chinese as default language
		lang := "zh-CN"
